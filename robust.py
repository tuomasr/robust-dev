...
from master_problem_dc_no_storage import CCMasterProblemNoStorage
...
def run_robust_optimization(master_problem_algorithm, subproblem_algorithm, output_dir):
    ...
    print("MASTER PROBLEM ALGORITHM", master_problem_algorithm)
    print("SUBPROBLEM ALGORITHM", subproblem_algorithm)
    print("NUM SCENARIOS", len(scenarios))
    print("STOCHASTIC", enable_stochastic)
    print("DISABLE EMISSION CONSTRAINT", disable_emission_constraint)
    ...
    elif master_problem_algorithm == "milp_dc_no_storage":
        master_problem_class = CCMasterProblemNoStorage
    ...
    elif subproblem_algorithm in ("miqp_dc_no_storage", "milp_dc_no_storage"):
        subproblem_milp = subproblem_algorithm == "milp_dc_no_storage"
        from subproblem_no_storage import (
            solve_subproblem,
            get_uncertain_variables,
            get_uncertainty_decisions,
        )
...
def main():
    ...
    parser.add_argument(
        "master_problem_algorithm",
        type=str,
        choices=("benders_dc", "dual_benders_dc", "milp_dc", "milp_dc_no_storage"),
    )
    parser.add_argument(
        "subproblem_algorithm", type=str, choices=("miqp_dc", "milp_dc", "miqp_dc_no_storage", "milp_dc_no_storage")
    )
    ...
    sys.stdout = MyLogger(args.output_dir, args.master_problem_algorithm, args.subproblem_algorithm)
