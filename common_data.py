...
avg_hourly_load_real_nodes = np.genfromtxt("avg_hourly_load.csv", delimiter=",", skip_header=1)
uncertainty_demand_increase_factor = 0.05
uncertainty_budget = 4.0

disable_emission_constraint = False
...
uncertainty_demand_increase = avg_hourly_load_real_nodes * uncertainty_demand_increase_factor
uncertainty_demand_increase = np.expand_dims(uncertainty_demand_increase, axis=0)
uncertainty_demand_increase = np.tile(uncertainty_demand_increase, (num_scenarios, num_years, 1))

assert uncertainty_demand_increase.shape == load.shape
...
for y in years:
    for o, (start, end) in enumerate(zip(start_indices, end_indices)):
        ...
        stoch = stoch / np.sum(stoch) * uncertainty_demand_increase[o, h1:h2, :] * load_growth_factor # * uncertainty_budget / float(num_real_nodes)

        uncertainty_demand_increase[o, h1:h2, :] *= load_growth_factor
        load[o, h1:h2, :] = load_slice * load_growth_factor + int(enable_stochastic) * stoch
...
if disable_emission_constraint:
    start_emissions = 999999999.9
    final_emissions = start_emissions
...
C_g += np.random.uniform(-1.0, 1.0, (num_scenarios, num_hours, 1))
C_g[:, :, :] = np.maximum(C_g[:, :, :], 0.0)
...
price_multiplier_x = 1.0e4
price_multiplier_y = 1.0e1

C_x = {
    (year, unit): (price_multiplier_x if disable_emission_constraint else 1.0) * 1.0e6
...
# Cost per project.
C_y = {
    (year, line): (price_multiplier_y if disable_emission_constraint else 1.0) * 1.0e9 * discount_factor ** (-year)
    for year in years
