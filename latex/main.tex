...
We augment the base system by having 10 time steps in the master problem ($t$) for making investments. For each master problem time step, we consider 24 time steps in the subproblem ($\tau$). Consequently, the subproblem has 240 time steps in total. We define variable load and wind as well as solar power data so that, for each master problem time step, we sample one day and use its hourly data for 24 consecutive subproblem time steps. We assume that load values increase by a factor of $D = 1.03$ at each master problem time step. Hydro power data such as initial storage levels ($S_{o, \tau, u}^0$) and inflows ($I_{o, \tau, u}$) have weekly granularity so we take the weekly value corresponding to each sampled day. In addition, we modify the system by introducing 3 equally weighted ($W_o = 1/3$) operating conditions by sampling uniform noise from $\textrm{U}(-100 \textrm{ MW}, 100 \textrm{ MW})$ to all generation ($G^{max}_{o, \tau, u}$) and transmission capacities ($F^{max}_{o, \tau, \ell}$ and $F^{min}_{o, \tau, \ell}$).

Gas, CCGT, oil, biomass, wind and solar power can be built at each (non-dummy) node. Each candidate unit has a maximum capacity ($G^{inv,max}_u$) of 10000 MW except for biomass units which we limit to 1000 MW due to limited fuel supply. In addition, neighboring (non-dummy) node pairs can be connected with two candidate DC transmission lines with both having 1000 MW of capacity to both directions. The investment cost parameters are shown in Table \ref{table_investment_parameters}. The upper bound for emissions for each master problem time step decreases linearly from $E_{0} = 100000 \textrm{ tonne}$ to $E_{9} = 20000 \textrm{ tonne}$.
...
Benders					& MIQP 				& 98047281.846 		& 2704.97				& 3				 \\
MILP					& MIQP 				& 98047281.846 		& 3975.56				& 3				 \\
\end{tabular}
\caption{}
\label{table_results}
\end{table}
