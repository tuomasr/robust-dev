...
theta = m.addVar(name="theta", lb=-999.999, ub=GRB.INFINITY)
...
def augment_master_problem(current_iteration, d):
    ...
    g, f, delta = add_primal_variables(v)
...
def get_investment_and_availability_decisions():
    ...
    def var_to_dict(var):
        return {key: value.x for key, value in var.items()}
...
# Emission prices can be computed from the master problem or subproblem. I use the subproblem
# currently.
"""
def get_emission_prices(g):
    i = g.keys()[0][-1]

    # Create a fixed LP model to access the dual variables.
    fixed_model = m
    emission_prices = np.zeros((len(scenarios), len(years)))

    for o in scenarios:
        for y in years:
            constraint_name = EMISSION_CONSTRAINTS_NAME + "_%d[%d,%d]" % (i, o, y)
            constraint = fixed_model.getConstrByName(constraint_name)
            emission_prices[o, y] = constraint.getAttr("Pi")
            # for constr in fixed_model.getConstrs():
            #     pi = constr.getAttr("Pi")

            #     if abs(pi) > 0:
            #         print(constr, pi)

    return emission_prices
"""
